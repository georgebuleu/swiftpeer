...
func (m *Message) Serialize() []byte {
	if m == nil {
		return make([]byte, 4) // 4-byte array for nil message
	}

	payloadLen := len(m.Payload) + 1 //1 byte for ID
	totalLen := 4 + payloadLen       // 4 bytes for length, rest for payload

	msg := make([]byte, totalLen)

	// write length of payload to the first 4 bytes
	binary.BigEndian.PutUint32(msg[0:4], uint32(payloadLen))

	// write id to the 5th byte
	msg[4] = byte(m.Id)

	// copy the payload starting from the 6th byte
